...
void Paravia::InitializePlayer(player *Me, int year, int city, int level, char *name, boolean MorF) {
    ...
    Me->Soldiers = 25;
    Me->TitleNum = 1;
    if (Me->MaleOrFemale == True) strcpy(Me->Title, MaleTitles[0]);
    else strcpy(Me->Title, FemaleTitles[0]);
...
int Paravia::AttackNeighbor(player *Me, player *Him) {
    int LandTaken;
    int deadsoldiers = 0;
    if (Me->WhichPlayer == 7) LandTaken = Random(9000) + 1000;
    else LandTaken = (Me->Soldiers * 1000) - (Me->Land / 3);
    if (LandTaken > (Him->Land - 5000)) LandTaken = (Him->Land - 5000) / 2;
    Me->Land += LandTaken;
    Him->Land -= LandTaken;
    printf("\a\n%s %s of %s invades and seizes %d hectares of land!\n", Me->Title, Me->Name, Me->City, LandTaken);
    deadsoldiers = Random(40);
    if (deadsoldiers > (Him->Soldiers - 15)) deadsoldiers = Him->Soldiers - 15;
    Him->Soldiers -= deadsoldiers;
    printf("%s %s loses %d soldiers in battle.\n", Him->Title, Him->Name, deadsoldiers);
    return(LandTaken);
}
void Paravia::BuyCathedral(player *Me) {
    Me->Cathedral += 1;
    Me->Clergy += Random(6);
    Me->Treasury -= 5000;
    Me->PublicWorks += 1.0;
...
void Paravia::BuyGrain(player *Me) {
    char string[256];
    int HowMuch;
...
void Paravia::BuyLand(player *Me) {
    char string[256];
    int HowMuch;
...
void Paravia::BuyMarket(player *Me) {
    Me->Marketplaces += 1;
    Me->Merchants += 5;
    Me->Treasury -= 1000;
    Me->PublicWorks += 1.0;
...
void Paravia::BuyMill(player *Me) {
    Me->Mills += 1;
    Me->Treasury -= 2000;
    Me->PublicWorks += 0.25;
    return;
}
void Paravia::BuyPalace(player *Me) {
    Me->Palace += 1;
    Me->Nobles += Random(2);
    Me->Treasury -= 3000;
    Me->PublicWorks += 0.5;
...
boolean Paravia::CheckNewTitle(player *Me) {
    int Total;
...
void Paravia::GenerateIncome(player *Me) {
    ...
    printf("%d\t\t%d\t\t%d\t\t%d %s\n", Me->CustomsDutyRevenue, Me->SalesTaxRevenue, Me->IncomeTaxRevenue, Me->JusticeRevenue, string);
    return;
}

void Paravia::ChangeTitle(player *Me) {
    if (Me->MaleOrFemale == True) strcpy(Me->Title, MaleTitles[Me->TitleNum]);
    else strcpy(Me->Title, FemaleTitles[Me->TitleNum]);
    if (Me->TitleNum == 7) {
        Me->IWon = True;
        return;
    }
...
void Paravia::NewLandAndGrainPrices(player *Me) {
    float x, y, MyRandom;
    int h;
...
int Paravia::ReleaseGrain(player *Me) {
    double xp;
    double zp;
    float x, z;
    char string[256];
    ...
    while (IsOK == False) {
        ...
        fgets(string, 255, stdin);
        HowMuch = (int)atoi(string);
        if (HowMuch == 1) HowMuch = Minimum;
        if (HowMuch == 2) HowMuch = Maximum;
    ...
    printf("(Press ENTER): ");
    fgets(string, 255, stdin);
    // Invade me is the trigger for losing land and goods. 
    // I'm not sure why the staggered ifs. Maybe to make it simpler for integer division?
    // 
...
void Paravia::SellGrain(player *Me) {
    char string[256];
    int HowMuch;
...
void Paravia::SellLand(player *Me) {
    char string[256];
    int HowMuch;
...
void Paravia::PrintInstructions(void) {
    char string[256];
...
void Paravia::NewTurn(player *Me, int HowMany, player MyPlayers[6], player *Baron) {
    ...
    CheckNewTitle(Me);
    Me->Year++;
    if (Me->Year == Me->YearOfDeath) ImDead(Me);
    if (Me->TitleNum >= 7) Me->IWon = True;
}
void Paravia::BuySellGrain(player *Me) {
    boolean Finished;
    char string[256];
    Finished = False;
    while (Finished == False) {
        ...
        printf("%d\t%d\t%d\t\t%.2f\t\t%d\n", Me->GrainReserve, Me->GrainDemand, Me->GrainPrice, Me->LandPrice, Me->Treasury);
        printf("steres\tsteres\t1000 st.\thectare\t\tgold florins\n");
        printf("\nYou have %d hectares of land.\n", Me->Land);
        printf("\n1. Buy grain, 2. Sell grain, 3. Buy land,");
        printf(" 4. Sell land\n(Enter q to continue): ");
        fgets(string, 255, stdin);
        if (string[0] == 'q') Finished = True;
        if (string[0] == '1') BuyGrain(Me);
        if (string[0] == '2') SellGrain(Me);
        if (string[0] == '3') BuyLand(Me);
        if (string[0] == '4') SellLand(Me);
    }
    return;
}
...
void Paravia::AdjustTax(player *Me) {
    ...
    AddRevenue(Me);
    if (Me->IsBankrupt == True) SeizeAssets(Me);
}
void Paravia::DrawMap(player *Me) {
    /* Not implemented yet. */
    return;
}
...
void Paravia::ImDead(player *Me) {
    ...
    int why;
    printf("\n\nVery sad news.\n%s %s has just died\n", Me->Title, Me->Name);
    if (Me->Year > 1450) printf("of old age after a long reign.\n");
    else {
        why = Random(8);
        switch (why) {
        case 0:
